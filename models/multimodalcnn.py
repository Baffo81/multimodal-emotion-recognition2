...
class EfficientFaceTemporal(nn.Module):
    ...
    def __init__(self, stages_repeats, stages_out_channels, num_classes=7, im_per_sample=25):
        ...
        self.conv5 = nn.Sequential(nn.Conv2d(input_channels, output_channels, 1, 1, 0, bias=False),
                                   nn.BatchNorm2d(output_channels),
                                   nn.ReLU(inplace=True), )
        self.conv1d_0 = conv1d_block(output_channels, 64, padding=1)
        self.conv1d_1 = conv1d_block(64, 64, padding=1)
        self.conv1d_2 = conv1d_block(64, 128, padding=1)
        self.conv1d_3 = conv1d_block(128, 128, padding=1)
    ...
    def forward_features(self, x):
        ...
        torch.cuda.empty_cache()  # Clear GPU cache after forward pass
        return x
...
class MultiModalCNN(nn.Module):
    def __init__(self, num_classes=8, fusion='ia', seq_length=15, pretr_ef='None', num_heads=1, last_ca=False):
        ...
        self.classifier_1 = nn.Sequential(
            AttentionBlock(in_dim_k=e_dim, in_dim_q=e_dim, out_dim=e_dim*2, num_heads=num_heads) if last_ca else nn.Identity(),
